...
function _updateAttributes(attributes, submodelElements, modelName) {
  if (attributes) {
    if (submodelElements && _isSubmodelField(attributes.type)) {
      ...
      attributes.submodel = _.clone(
        _getSubmodel(submodelElements, submodelName),
      );
...
function _updateAllAttributes(elements, submodelElements, modelName) {
  for (const element of elements) {
    element.attributes &&
      _updateAttributes(element.attributes, submodelElements, modelName);
...
function formatJson(json, fileName) {
  ...
  const formatedSubmodels =
    submodelsDef && _formatSubmodel(submodelsDef.elements, modelName);
...
function xml2json() {
  return mapStream(function(file, cb) {
    ...
    if (file.isStream()) {
      return cb(
        new gutil.PluginError('gulp-xml2json', 'Streaming not supported'),
      );
    }

    if (['.xml', '.XML'].indexOf(path.extname(file.path)) === -1) {
      gutil.log(
        'gulp-xml2json: Skipping unsupported xml ' +
...
gulp.task('default', gulp.series('compile-definition'));
